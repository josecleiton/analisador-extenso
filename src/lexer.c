            ...
            if (!strcmp (ref.name, ctx->cursor) || matchPlural (ctx, i, ref.name))
                {
                    ...
                    if (isdigit (valorTk))
                        {
                            ctx->token = valorTk;
                            while (*ctx->cursor && (isalpha (*ctx->cursor) || *ctx->cursor == ' '))
                                ctx->cursor++;
                            *ctx->cursor = trade;
                            ctx->tokenType = NUM;
                            ctx->isNumber = true;
...
{
    if (*ctx->cursor != 'a' && *ctx->cursor != 'f' && *ctx->cursor != 'd' && *ctx->cursor != 'e')
        return;
    else if (!strcmp (ctx->cursor, (const char *)"abre") || !strcmp (ctx->cursor, (const char *)"fecha") || !strcmp (ctx->cursor, (const char *)"dividido") || !strcmp (ctx->cursor, (const char *)"fatorial") || !strcmp (ctx->cursor, (const char *)"elevado"))
        {
            int i = 0;
            ctx->cursor[*k] = '-';
            ...
            *temp = ctx->cursor[i];
            ctx->cursor[i] = '\0';
            if (strcmp (&ctx->cursor[*k + 1], (const char *)"parentese") && strcmp (&ctx->cursor[*k + 1], (const char *)"de") && strcmp (&ctx->cursor[*k + 1], (const char *)"por") && strcmp (&ctx->cursor[*k + 1], (const char *)"a"))
                {
                    ctx->cursor[*k] = ' ';
                    reportError (ctx, 0);
                    return;
